...
def display_results(df, ticker, days_to_predict, future_prices):
    ...
    try:
        ...
        forecast_df['Trạng thái Raw'] = forecast_df.apply(get_status, axis=1)
        forecast_df['Trạng thái'] = forecast_df['Trạng thái Raw'].apply(lambda x: f"▲ {x}" if x == 'TĂNG' else f"▼ {x}")
        
        display_df = forecast_df[['Ngày', 'Giá đóng cửa dự đoán', 'Trạng thái', 'Trạng thái Raw']].copy()
        display_df['Ngày'] = pd.to_datetime(display_df['Ngày']).dt.strftime('%d/%m/%Y')
        
        def style_rows(row):
            styles = ['' for _ in row]
            if row['Trạng thái Raw'] == 'TĂNG':
                styles[list(row.index).index('Trạng thái')] = 'color: green; font-weight: bold;'
            else:
                styles[list(row.index).index('Trạng thái')] = 'color: red; font-weight: bold;'
            return styles

        styled_df = display_df.set_index('Ngày').style.apply(style_rows, axis=1).format({
            "Giá đóng cửa dự đoán": "${:,.2f}"
        }).hide(subset=['Trạng thái Raw'], axis='columns')
...
st.info("Ứng dụng này sử dụng mô hình CNN-BiLSTM để dự đoán giá cổ phiếu.")

col1, col2 = st.columns([1, 2])
with col1:
    st.header("Cài đặt")
    ticker_input = st.text_input("Nhập mã chứng khoán (VD: VCB.VN, ^VNINDEX, AAPL):", "AAPL").upper()
