...
def add_technical_indicators(df):
    ...
    # FIX: Ensure columns are numeric before calculations
    for col in ['Open', 'High', 'Low', 'Close', 'Volume']:
        df_ta[col] = pd.to_numeric(df_ta[col], errors='coerce')
    
    # Drop rows with non-numeric data that were coerced to NaN
    df_ta.dropna(inplace=True)

    df_ta['RSI'] = calculate_rsi(df_ta)
    df_ta['MACD'] = calculate_macd(df_ta)
    
    # Drop rows with NaN values created by TA calculations
    df_ta.dropna(inplace=True)
    return df_ta
...
def display_results(df, ticker, days_to_predict, future_prices):
    ...
    try:
        ...
        forecast_df = pd.DataFrame({'Ngày': future_dates_table, 'Giá đóng cửa dự đoán': future_prices})
        forecast_df['Ngày'] = pd.to_datetime(forecast_df['Ngày']).dt.strftime('%d/%m/%Y')
        st.dataframe(forecast_df.set_index('Ngày').style.format({"Giá đóng cửa dự đoán": "${:,.2f}"}), use_container_width=True)
    except Exception as e:
